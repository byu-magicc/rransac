...
typedef typename Transformation::Measurement Measurement;
typedef Cluster<DataType,TransformDataType> ClusterT;
typedef DataTreeClusters<DataType,TransformDataType> DataTreeClustersT;



    Parameters params_;                                        /** < System parameters */
    SourceContainer source_container_;                         /** < Contains all of the instantiated sources. */
    std::list<Measurement> new_meas_;     /** < Contains all of the new measurements. */
    ...
    DataTreeClustersT data_tree_;                               /** < Contains measurements that are not in a consensus set */
    std::vector<typename std::list<ClusterT>::iterator> clusters_;       /** < Iterators to clusters. RANSAC tries to form measurements from each clusters */
