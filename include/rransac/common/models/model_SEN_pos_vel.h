...
Mat GetLinTransFuncMatState(const State& state, const double dt) {  
    Eigen::Matrix<double, g_dim_,g_dim_> tmp = (this->state_.u_*dt).Jr()*dt;  
    this->F_.block(0,g_dim_,g_dim_,g_dim_) = typename State::g_type_(State::u_type_::Exp(this->state_.u_.data_*dt)).Adjoint();
...
void UpdateState(const Eigen::Matrix<double,cov_dim_,1>& state_update) {
    ...
    twist_update.block(g_dim_,0, State::u_type_::dim_a_vel_,1) = state_update.block(g_dim_+1,0,State::u_type_::dim_a_vel_,1);
