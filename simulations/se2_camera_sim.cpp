...
CameraSimR2::CameraSimR2(const CameraData& camera_data, double dt, double end_time, int num_tracks) : camera_data_(camera_data) {
    ...
    params.cluster_velocity_threshold_ = 0.04;
...
typename CameraSimR2::State_ CameraSimR2::GenerateRandomState() {
...
bool CameraSimR2::InsurveillanceRegion(const State_& state) {
...
void CameraSimR2::Propagate(double start_time, double end_time, Stats<Model_>& stats) {
    ...
    for (double ii =start_time; ii < end_time; ii += this->dt_) {
        ...
        for (auto& track : tracks_ ) {
            ...
            if (ii !=this->start_time_) {
                track.state_.u_.data_ += noise_mat_*sqrt(this->process_noise_)*rransac::utilities::GaussianRandomGenerator(Algebra_::dim_)*this->dt_;
            ...
            if (fabs(rand_num(0,0)) < this->sys_->source_container_.GetParams(this->m_.source_index).probability_of_detection_) {

                tmp1 = this->sys_->source_container_.GenerateRandomMeasurement(this->m_.source_index, MatR_::Identity()*sqrt(this->meas_noise_),track.state_,transform_state,EmptyMat);
        ...
        for (int jj =0; jj < this->camera_data_.num_false_meas; ++jj) {
            ...
            tmp1 = this->sys_->source_container_.GenerateRandomMeasurement(this->m_.source_index, MatR_::Identity()*sqrt(this->meas_noise_),rand_state,transform_state,EmptyMat);
        ...
        stats.CalculatePerformanceMeasures(sys_,tracks_,0);
...
Stats<typename CameraSimR2::Model_> stats;
stats.Reset();
